...
/**
 * err_or - prints the error output of a file
...
int *err_or(char *argv, int count, char *args)
{
	...
	number = i_toa(count, 10);

	write(2, argv, strlen(argv));
	write(2, ": ", 2);
	write(2, number, strlen(number));
	write(2, ": ", 2);
...
int stat_(char **cmd, char **menged)
{
	...
	{
		concat_str = _strconct(menged[kotari], "/");
		new_concat = _strconct(concat_str, cmd[0]);
